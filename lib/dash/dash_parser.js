...
shaka.dash.DashParser = class {
  ...
  constructor() {
    ...
    /**
     * Flag to track if an MPD expiration update (validity expiration event)
     * has occurred. When true, periodic updates from minimumUpdatePeriod
     * are disabled.
     * @private {boolean}
     */
    this.mpdExpireUpdateReceived_ = false;
  ...
  async updateFromData(data) {
    try {
      ...
      const finalManifestUri = this.manifestUris_[0];

      // Parse the manifest from the provided data
      await this.parseManifest_(data, finalManifestUri, 'MPD');

      // Keep track of how long the update took
      ...
      // Set flag to disable periodic updates from minimumUpdatePeriod
      // when MPD expiration update is received
      this.mpdExpireUpdateReceived_ = true;

      // Stop the periodic update timer since validity expiration takes over
  ...
  setUpdateTimer_(offset) {
    ...
    // If an MPD expiration update has been received, disable periodic updates
    // from minimumUpdatePeriod since validity expiration takes precedence
    if (this.mpdExpireUpdateReceived_) {
      return;
    }
