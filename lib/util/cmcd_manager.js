...
shaka.util.CmcdManager = class {
   ...
   * Sends the CMCD data in the body of a POST request.
   *
   * @param {!CmcdData} cmcdData The CMCD data to send.
   * @param {shaka.extern.CmcdTarget} target The CMCD target configuration.
   * @private
   */
  sendCmcdRequest_(cmcdData, target) {
    const retryParams = shaka.net.NetworkingEngine.defaultRetryParameters();
    const baseURL = target.url;

    let request = shaka.net.NetworkingEngine.makeRequest([baseURL], retryParams);
    const requestBody = shaka.util.CmcdManager.toQuery(cmcdData);
  ...
  static serialize(data) {
    ...
    for (const key of keys) {
      ...
      if (type === 'string' && !shaka.util.CmcdManager.CmcdKeys.KeysWithTokens.includes(key)) {      
...
shaka.util.CmcdManager.CmcdKeys = {
  ...
  KeysWithTokens: [  // Keys that have a token value, not to be surrounded by quotes
    'e', 'ot', 'sf', 'st', 'sta'
  ]
};
