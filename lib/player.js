...
/**
 * @event shaka.Player.MpdExpireUpdateEvent
 * @description Fired when an emsg box with scheme 'urn:mpeg:dash:event:2012'
 *   and value '3' is found, containing an updated MPD manifest.
 * @property {string} type
 *   'mpdexpireupdate'
 * @property {BufferSource} manifest
 *   The new manifest data from the emsg message data.
 * @exportDoc
 */
...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async makePreloadManager_(assetUri, startTime, mimeType, preloadConfig,
    ...
    const manifestPlayerInterface = {
      ...
      onMpdExpireUpdate: (manifestData) => {
        const eventName = shaka.util.FakeEvent.EventName.MpdExpireUpdate;
        const data = (new Map()).set('manifest', manifestData);
        preloadManager.dispatchEvent(shaka.Player.makeEvent_(eventName, data));
        this.onMpdExpireUpdate_(manifestData);
      },
  ...
  /**
   * Callback from MediaSourceEngine when an MPD expiry event with value=3
   * is received containing new manifest data.
   *
   * @param {BufferSource} manifestData
   * @private
   */
  onMpdExpireUpdate_(manifestData) {
    if (this.parser_ && this.parser_.updateFromData) {
      this.parser_.updateFromData(manifestData);
    }
  }
